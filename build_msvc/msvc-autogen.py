...
def parse_config_into_rvt_config():
    ...
    config_dict["PACKAGE_STRING"] = f"\"Revolt Core {version}\""

    with open(os.path.join(SOURCE_DIR,'../build_msvc/revolt_config.h.in'), "r", encoding="utf8") as template_file:
    ...
    with open(os.path.join(SOURCE_DIR,'../build_msvc/revolt_config.h'), "w", encoding="utf8") as rvt_config:
        rvt_config.writelines(template)
...
def main():
    parser = argparse.ArgumentParser(description='Revolt-core msbuild configuration initialiser.')
    ...
    parse_config_into_rvt_config()
    copyfile(os.path.join(SOURCE_DIR,'../build_msvc/revolt_config.h'), os.path.join(SOURCE_DIR, 'config/revolt-config.h'))
