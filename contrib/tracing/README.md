...
Tracing (USDT) support for Revolt Core on Linux using. For more information on
USDT support in Revolt Core see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `revoltd` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `revoltd` binary. The paths in the examples can be changed, for
example, to point to release builds if needed. See the
[Revolt Core USDT documentation] on how to list available tracepoints in your
`revoltd` binary.

[Revolt Core USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
```
$ python3 contrib/tracing/p2p_monitor.py ./src/revoltd
```
...
Revolt P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py ./src/revoltd
```
...
In a different terminal, starting Revolt Core in SigNet mode and with
re-indexing enabled.

```
$ ./src/revoltd -signet -reindex
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py ./src/revoltd
```
