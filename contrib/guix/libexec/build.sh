...
make -C depends --jobs="$JOBS" HOST="$HOST" \
                                   ...
                                   qt_config_opts_x86_64_linux='-platform linux-g++ -xplatform revolt-linux-g++' \
...
(
    ...
    case "$HOST" in
        *mingw*)
            make deploy ${V:+V=1} REVOLT_WIN_INSTALLER="${OUTDIR}/${DISTNAME}-win64-setup-unsigned.exe"
            ;;
    esac

    # Setup the directory where our Revolt Core build for HOST will be
    # installed. This directory will also later serve as the input for our
    # binary tarballs.
    INSTALLPATH="${PWD}/installed/${DISTNAME}"
    mkdir -p "${INSTALLPATH}"
    # Install built Revolt Core to $INSTALLPATH
    case "$HOST" in
        *darwin*)
    ...
    (
        ...
        # copy over the example revolt.conf file. if contrib/devtools/gen-revolt-conf.sh
        # has not been run before buildling, this file will be a stub
        cp "${DISTSRC}/share/examples/revolt.conf" "${DISTNAME}/"
