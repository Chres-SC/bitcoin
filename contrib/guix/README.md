# Bootstrappable Revolt Core Builds

This directory contains the files necessary to perform bootstrappable Revolt
Core builds.
...
To build Revolt Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Revolt Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/revolt-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/revolt-core/guix.sigs).
