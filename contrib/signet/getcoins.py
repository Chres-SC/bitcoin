...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing revolt-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='revolt-cli', help='revolt-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Revolt address to which the faucet should send')
...
parser.add_argument('revolt_cli_args', nargs='*', help='Arguments to pass on to revolt-cli (default: -signet)')

args = parser.parse_args()

if args.revolt_cli_args == []:
    args.revolt_cli_args = ['-signet']


def revolt_cli(rpc_command_and_params):
    argv = [args.cmd] + args.revolt_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = revolt_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = revolt_cli(['getnewaddress', 'faucet', 'bech32'])
