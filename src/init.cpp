 ...
 */
static const char* BITCOIN_PID_FILENAME = "revoltd.pid";
...
void SetupServerArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-conf=<file>", strprintf("Specify path to read-only configuration file. Relative paths will be prefixed by datadir location (only useable from command line, not configuration file) (default: %s)", REVOLT_CONF_FILENAME), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    ...
    argsman.AddArg("-settings=<file>", strprintf("Specify path to dynamic settings data file. Can be disabled with -nosettings. File is written at runtime and not meant to be edited by users (use %s instead for custom settings). Relative paths will be prefixed by datadir location. (default: %s)", REVOLT_CONF_FILENAME, REVOLT_SETTINGS_FILENAME), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
#if HAVE_SYSTEM
    ...
    argsman.AddArg("-sandbox=<mode>", "Use the experimental syscall sandbox in the specified mode (-sandbox=log-and-abort or -sandbox=abort). Allow only expected syscalls to be used by revoltd. Note that this is an experimental new feature that may cause revoltd to exit or crash unexpectedly: use with caution. In the \"log-and-abort\" mode the invocation of an unexpected syscall results in a debug handler being invoked which will log the incident and terminate the program (without executing the unexpected syscall). In the \"abort\" mode the invocation of an unexpected syscall results in the entire process being killed immediately by the kernel without executing the unexpected syscall.", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
...
bool AppInitParameterInteraction(const ArgsManager& args, bool use_syscall_sandbox)
{
    ...
    if (args.IsArgSet("-sandbox") && !args.IsArgNegated("-sandbox")) {
        ...
        LogPrintf("Experimental syscall sandbox enabled (-sandbox=%s): revoltd will terminate if an unexpected (not allowlisted) syscall is invoked.\n", sandbox_arg);
    }
#endif // USE_SYSCALL_SANDBOX
...
bool AppInitMain(NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    const auto BadPortWarning = [](const char* prefix, uint16_t port) {
        return strprintf(_("%s request to listen on port %u. This port is considered \"bad\" and "
                           "thus it is unlikely that any Revolt Core peers connect to it. See "
