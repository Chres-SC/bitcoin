...
static RPCHelpMan echo(const std::string& name)
{
    return RPCHelpMan{name,
                ...
                "revolt-cli and the GUI. There is no server-side difference.",
                {
    ...
    return RPCHelpMan{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue {
            ...
            if (interfaces::Ipc* ipc = local_init.ipc()) {
                // Spawn a new revolt-node process and call makeEcho to get a
                ...
                // and spawn bitcoin-echo below instead of revolt-node. But
                // using revolt-node avoids the need to build and install a
                // new executable just for this one test.
                auto init = ipc->spawnProcess("revolt-node");
                echo = init->makeEcho();
                ipc->addCleanup(*echo, [init = init.release()] { delete init; });
            } else {
                // IPC support is not available because this is a revoltd
                // process not a revoltd-node process, so just create a local
