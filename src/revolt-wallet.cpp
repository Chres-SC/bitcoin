...
static bool WalletAppInit(ArgsManager& args, int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(args) || args.IsArgSet("-version")) {
        std::string strUsage = strprintf("%s revolt-wallet version", PACKAGE_NAME) + " " + FormatFullVersion() + "\n";

        if (args.IsArgSet("-version")) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                        "revolt-wallet is an offline tool for creating and interacting with " PACKAGE_NAME " wallet files.\n"
                        "By default revolt-wallet will act on wallets in the default mainnet wallet directory in the datadir.\n"
                        "To change the target wallet, use the -datadir, -wallet and -regtest/-signet/-testnet arguments.\n\n"
                        "Usage:\n"
                        "  revolt-wallet [options] <command>\n";
            strUsage += "\n" + args.GetHelpMessage();
        }
...
MAIN_FUNCTION
{
    ...
    if (!command) {
        tfm::format(std::cerr, "No method provided. Run `revolt-wallet -help` for valid methods.\n");
        return EXIT_FAILURE;
    }
