...
Currently, it is possible to create a multisig wallet using Revolt core only.
...
At the time of writing, there is no way to extract a specific path from wallets in Revolt core. For this, an external signer/xpub can be used.
...
for ((n=1;n<=3;n++))
do
 ./src/revolt-cli -signet createwallet "participant_${n}"
done
```

Extract the xpub of each wallet. To do this, the `listdescriptors` RPC is used. By default, Revolt core single-sig wallets are created using path `m/44'/1'/0'` for PKH, `m/84'/1'/0'` for WPKH, `m/49'/1'/0'` for P2WPKH-nested-in-P2SH and `m/86'/1'/0'` for P2TR based accounts. Each of them uses the chain 0 for external addresses and chain 1 for internal ones, as shown in the example below.
...
All RPCs in Revolt core will include the checksum in their output.
...
for ((n=1;n<=3;n++))
do
 xpubs["internal_xpub_${n}"]=$(./src/revolt-cli -signet -rpcwallet="participant_${n}" listdescriptors | jq '.descriptors | [.[] | select(.desc | startswith("wpkh") and contains("/1/*"))][0] | .desc' | grep -Po '(?<=\().*(?=\))')

 xpubs["external_xpub_${n}"]=$(./src/revolt-cli -signet -rpcwallet="participant_${n}" listdescriptors | jq '.descriptors | [.[] | select(.desc | startswith("wpkh") and contains("/0/*") )][0] | .desc' | grep -Po '(?<=\().*(?=\))')
done
```
...
external_desc_sum=$(./src/revolt-cli -signet getdescriptorinfo $external_desc | jq '.descriptor')
internal_desc_sum=$(./src/revolt-cli -signet getdescriptorinfo $internal_desc | jq '.descriptor')
...
Documentation for these and other parameters can be found by typing `./src/revolt-cli help importdescriptors`.
...
```bash
./src/revolt-cli -signet -named createwallet wallet_name="multisig_wallet_01" disable_private_keys=true blank=true

./src/revolt-cli  -signet -rpcwallet="multisig_wallet_01" importdescriptors "$multisig_desc"

./src/revolt-cli  -signet -rpcwallet="multisig_wallet_01" getwalletinfo
```
...
```bash
for ((n=1;n<=3;n++)); do ./src/revolt-cli -signet loadwallet "participant_${n}"; done
```
...
```bash
receiving_address=$(./src/revolt-cli -signet -rpcwallet="multisig_wallet_01" getnewaddress)

./contrib/signet/getcoins.py -c ./src/revolt-cli -a $receiving_address
```
...
```bash
./src/revolt-cli -signet -rpcwallet="multisig_wallet_01" getbalances
```
...
```bash
balance=$(./src/revolt-cli -signet -rpcwallet="multisig_wallet_01" getbalance)

amount=$(echo "$balance * 0.8" | bc -l | sed -e 's/^\./0./' -e 's/^-\./-0./')

destination_addr=$(./src/revolt-cli -signet -rpcwallet="participant_1" getnewaddress)

funded_psbt=$(./src/revolt-cli -signet -named -rpcwallet="multisig_wallet_01" walletcreatefundedpsbt outputs="{\"$destination_addr\": $amount}" | jq -r '.psbt')
```
...
```bash
./src/revolt-cli -signet decodepsbt $funded_psbt

./src/revolt-cli -signet analyzepsbt $funded_psbt
```
...
```bash
psbt_1=$(./src/revolt-cli -signet -rpcwallet="participant_1" walletprocesspsbt $funded_psbt | jq '.psbt')

psbt_2=$(./src/revolt-cli -signet -rpcwallet="participant_2" walletprocesspsbt $funded_psbt | jq '.psbt')
```
...
```bash
combined_psbt=$(./src/revolt-cli -signet combinepsbt "[$psbt_1, $psbt_2]")
```
...
```bash
finalized_psbt_hex=$(./src/revolt-cli -signet finalizepsbt $combined_psbt | jq -r '.hex')

./src/revolt-cli -signet sendrawtransaction $finalized_psbt_hex
```
...
```bash
psbt_1=$(./src/revolt-cli -signet -rpcwallet="participant_1" walletprocesspsbt $funded_psbt | jq -r '.psbt')

psbt_2=$(./src/revolt-cli -signet -rpcwallet="participant_2" walletprocesspsbt $psbt_1 | jq -r '.psbt')

finalized_psbt_hex=$(./src/revolt-cli -signet finalizepsbt $psbt_2 | jq -r '.hex')
