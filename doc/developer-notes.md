...
- [Development guidelines](#development-guidelines)
    - [General Revolt core](#general-revolt-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Revolt core uses [Doxygen](https://www.doxygen.nl/) to generate its official documentation.
...
Revolt core is a multi-threaded application, and deadlocks or other
...
i.e. `CPPFLAGS="-DDEBUG_LOCKCONTENTION"`, then build and run revoltd.

You can then use the `-debug=lock` configuration option at revoltd startup or
`revolt-cli logging '["lock"]'` at runtime to turn on lock contention logging.
It can be toggled off again with `revolt-cli logging [] '["lock"]'`.
...
```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_revolt
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_revolt --log_level=test_suite
$ valgrind -v --leak-check=full src/revoltd -printtoconsole
...
# A coverage report will now be accessible at `./test_revolt.coverage/index.html`.
```
...
To profile a running revoltd process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep revoltd` -- sleep 60
```
...
Revolt core can be compiled with various "sanitizers" enabled, which add
...
- [Main thread (`revoltd`)](https://doxygen.bitcoincore.org/revoltd_8cpp.html#a0ddf1224851353fc92bfbff6f499fa97)
  : Started from `main()` in `revoltd.cpp`. Responsible for starting up and
...
However, in open source software such as Revolt core, where everyone uses
...
pay attention to for reviewers of Revolt core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: Revolt core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
Some of these are maintained by active developers of Revolt core, in which case
...
a Revolt core subtree, so that they can be integrated quickly. Cosmetic changes
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `revolt-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `revolt-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Revolt core, the wallet was always
