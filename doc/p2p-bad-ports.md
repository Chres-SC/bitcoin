When Revolt core automatically opens outgoing P2P connections, it chooses
...
A malicious actor may gossip an address:port where no Revolt node is listening,
or one where a service is listening that is not related to the Revolt network.
...
and usually require authentication. A connection attempt (by Revolt core,
trying to connect because it thinks there is a Revolt node on that
...
Below is a list of "bad" ports which Revolt core avoids when choosing a peer to
