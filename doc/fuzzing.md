# Fuzzing Revolt core using libFuzzer

## Quickstart guide

To quickly get started fuzzing Revolt core using [libFuzzer](https://llvm.org/docs/LibFuzzer.html):
...
## Overview of Revolt core fuzzing

[Google](https://github.com/google/fuzzing/) has a good overview of fuzzing in general, with contributions from key architects of some of the most-used fuzzers. [This paper](https://agroce.github.io/bitcoin_report.pdf) includes an external overview of the status of Revolt core fuzzing, as of summer 2021.  [John Regehr](https://blog.regehr.org/archives/1687) provides good advice on writing code that assists fuzzers in finding bugs, which is useful for developers to keep in mind.
...
It is possible to specify `revoltd` arguments to the `fuzz` executable.
...
The project's collection of seed corpora is found in the [`revolt-core/qa-assets`](https://github.com/bitcoin-core/qa-assets) repo.

To fuzz `process_message` using the [`revolt-core/qa-assets`](https://github.com/bitcoin-core/qa-assets) seed corpus:
...
If you find coverage increasing inputs when fuzzing you are highly encouraged to submit them for inclusion in the [`revolt-core/qa-assets`](https://github.com/bitcoin-core/qa-assets) repo.

Every single pull request submitted against the Revolt core repo is automatically tested against all inputs in the [`revolt-core/qa-assets`](https://github.com/bitcoin-core/qa-assets) repo. Contributing new coverage increasing inputs is an easy way to help make Revolt core more robust.
...
with certain assembly code from Revolt core's code. See [developer notes on sanitizers](https://github.com/bitcoin/bitcoin/blob/master/doc/developer-notes.md#sanitizers)
...
# Fuzzing Revolt core using afl++

## Quickstart guide

To quickly get started fuzzing Revolt core using [afl++](https://github.com/AFLplusplus/AFLplusplus):
...
# Fuzzing Revolt core using Honggfuzz

## Quickstart guide

To quickly get started fuzzing Revolt core using [Honggfuzz](https://github.com/google/honggfuzz):
...
## Fuzzing the Revolt core P2P layer using Honggfuzz NetDriver

Honggfuzz NetDriver allows for very easy fuzzing of TCP servers such as Bitcoin
Core without having to write any custom fuzzing harness. The `revoltd` server
...
$ git apply << "EOF"
diff --git a/src/revoltd.cpp b/src/revoltd.cpp
...
$ honggfuzz/honggfuzz --exit_upon_crash --quiet --timeout 4 -n 1 -Q \
      -E HFND_TCP_PORT=18444 -f inputs/ -- \
          src/revoltd -regtest -discover=0 -dns=0 -dnsseed=0 -listenonion=0 \
                       -nodebuglogfile -bind=127.0.0.1:18444 -logthreadnames \
                       -debug
```

# Fuzzing Revolt core using Eclipser (v1.x)

## Quickstart guide

To quickly get started fuzzing Revolt core using [Eclipser v1.x](https://github.com/SoftSec-KAIST/Eclipser/tree/v1.x):
...
Revolt core participates in Google's [OSS-Fuzz](https://github.com/google/oss-fuzz/tree/master/projects/bitcoin-core)
