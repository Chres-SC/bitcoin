...
`./`               | `bitcoin.conf`        | User-defined [configuration settings](bitcoin-conf.md) for `revoltd` or `bitcoin-qt`. File is not written to by the software and must be created manually. Path can be specified by `-conf` option
`./`               | `revoltd.pid`        | Stores the process ID (PID) of `revoltd` or `bitcoin-qt` while running; created at start and deleted on shutdown; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `revoltd` or `bitcoin-qt`; can be specified by `-debuglogfile` option
