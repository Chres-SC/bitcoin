# PSBT Howto for Revolt Core

Since Revolt Core 0.17, an RPC interface exists for Partially Signed Bitcoin
...
PSBT is an interchange format for Revolt Transactions that are not fully signed
...
Overall, the construction of a fully signed Revolt Transaction goes through the
following steps:
...
#### Multisig with multiple Revolt Core instances
...
Revolt Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `revolt-cli` is used.
...
Later, when *V* RVT has been received on *Amulti*, and Bob and Carol want to
