# CJDNS support in Revolt Core

It is possible to run Revolt Core over CJDNS, an encrypted IPv6 network that
...
done outside of Revolt Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Revolt network and individual nodes.
...
before it will work with your Revolt Core node. This is described in steps
...
## Run Revolt Core with CJDNS

Once you are connected to the CJDNS network, the following Revolt Core
...
When enabled, this option tells Revolt Core that it is running in an
...
IPv6 local network. This helps Revolt Core perform better address management:
...
CJDNS support was added to Revolt Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `revolt-cli -addrinfo` to see the
...
## CJDNS-related information in Revolt Core

There are several ways to see your CJDNS address in Revolt Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `revolt-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `revolt-cli getpeerinfo`).
