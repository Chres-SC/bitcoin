Revolt Core version *0.15.0* is now available from:
...
- The `estimatesmartfee` RPC interface has been changed (See [PR 10707](https://github.com/bitcoin/bitcoin/pull/10707)):
    ...
    - If Revolt Core has not been running for long enough and has not seen enough blocks or transactions to produce an accurate fee estimation, an error will be returned (previously a value of -1 was used to indicate an error, which could be confused for a feerate).
...
Revolt Core 0.15.0 contains the following changes to the RPC interface and `revolt-cli` for multi-wallet:

* When running Revolt Core with a single wallet, there are **no** changes to the RPC interface or `revolt-cli`. All RPC calls and `revolt-cli` commands continue to work as before.
* When running Revolt Core with multi-wallet, all *node-level* RPC methods continue to work as before. HTTP RPC requests should be send to the normal `<RPC IP address>:<RPC port>` endpoint, and `revolt-cli` commands should be run as before. A *node-level* RPC method is any method which does not require access to the wallet.
* When running Revolt Core with multi-wallet, *wallet-level* RPC methods must specify the wallet for which they're intended in every request. HTTP RPC requests should be send to the `<RPC IP address>:<RPC port>/wallet/<wallet name>` endpoint, for example `127.0.0.1:8332/wallet/wallet1.dat`. `revolt-cli` commands should be run with a `-rpcwallet` option, for example `revolt-cli -rpcwallet=wallet1.dat getbalance`.
...
Revolt Core has supported creating opt-in replace-by-fee (RBF) transactions
...
In previous versions of Revolt Core, a portion of each block could be reserved for transactions based on the age and value of UTXOs they spent. This concept (Coin Age Priority) is a policy choice by miners, and there are no consensus rules around the inclusion of Coin Age Priority transactions in blocks. In practice, only a few miners continue to use Coin Age Priority for transaction selection in blocks. Revolt Core 0.15 removes all remaining support for Coin Age Priority (See [PR 9602](https://github.com/bitcoin/bitcoin/pull/9602)). This has the following implications:
...
- `uptime` returns the total runtime of the `revoltd` server since its last start (See [PR 10400](https://github.com/bitcoin/bitcoin/pull/10400)).

Low-level RPC changes
---------------------

- When using Revolt Core in multi-wallet mode, RPC requests for wallet methods must specify
...
- The second argument in `prioritisetransaction` has been renamed from `priority_delta` to `dummy` since Revolt Core no longer has a concept of coin age priority. The `dummy` argument has no functional effect, but is retained for positional argument compatibility. See [Removal of Coin Age Priority](#removal-of-coin-age-priority).
...
- Error codes have been updated to be more accurate for the following error cases (See [PR 9853](https://github.com/bitcoin/bitcoin/pull/9853)):
  ...
  - `removeprunedfunds` now returns RPC_WALLET_ERROR if `revoltd` is unable to remove
  the transaction. Previously returned RPC_INTERNAL_ERROR.
  - `removeprunedfunds` now returns RPC_INVALID_PARAMETER if the transaction does not
  exist in the wallet. Previously returned RPC_INTERNAL_ERROR.
  - `fundrawtransaction` now returns RPC_INVALID_ADDRESS_OR_KEY if an invalid change
  address is provided. Previously returned RPC_INVALID_PARAMETER.
  - `fundrawtransaction` now returns RPC_WALLET_ERROR if `revoltd` is unable to create
...
- #10475 `39039b1` [RPC] getmempoolinfo mempoolminfee is a RVT/KB feerate (instagibbs)
...
- #9672 `46311e7` Opt-into-RBF for RPC & revolt-tx (luke-jr)
...
- #10400 `1680ee0` [RPC] Add an uptime command that displays the amount of time (in seconds) revoltd has been running (rvelhote)
- #10683 `d81bec7` rpc: Move the `generate` RPC call to rpcwallet (laanwj)
- #10710 `30bc0f6` REST/RPC example update (Mirobit)
- #10747 `9edda0c` [rpc] fix verbose argument for getblock in revolt-cli (jnewbery)
...
- #9805 `5b583ef` Add seed.rvt.petertodd.org to mainnet DNS seeds (petertodd)
...
- #10017 `02d64bd` combine_logs.py - aggregates log files from multiple revoltds during functional tests (jnewbery)
...
- #10142 `ed09dd3` Run revolt_test-qt under minimal QPA platform (ryanofsky)
...
- #10636 `4bc853b` [qa] util: Check return code after closing revoltd proc (MarcoFalke)
...
- #10447 `f259263` Make revoltd invalid argument error message specific (laanwj)
- #10495 `6a38b79` contrib: Update location of seeds.txt (laanwj)
- #10469 `b6b150b` Fixing typo in rpcdump.cpp help message (keystrike)
- #10451 `27b9931` contrib/init/revoltd.openrcconf: Don't disable wallet by default (luke-jr)
