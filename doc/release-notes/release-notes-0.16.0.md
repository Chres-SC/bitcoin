Revolt Core version 0.16.0 is now available from:
...
Revolt Core now has more flexibility in where the wallets directory can be
...
------------------------------------
The minimum version of the GCC compiler required to compile Revolt Core is now 4.8. No effort will be
...
GUI changes
-----------
- Uses of "µRVT" in the GUI now also show the more colloquial term "bits", specified in BIP176.
...
- revolt-cli now has an `-stdinrpcpass` option to allow the RPC password to be read from standard input.
- The `-usehd` option has been removed.
- revolt-cli now supports a new `-getinfo` flag which returns an output like that of the now-removed `getinfo` RPC.
...
- #11970 `b7450cd` Add test coverage for revolt-cli multiwallet calls (ryanofsky)
...
- #10997 `affe927` Add option -stdinrpcpass to revolt-cli to allow RPC password to be read from standard input (jharvell)
...
- #12035 `eeb6d52` Change µRVT to bits (jb55)
...
- #11621 `1f7695b` [build] Add temp_revolt_locale_qrc to CLEAN_QT to fix make distcheck (fanquake)
...
- #11711 `6378e5c` revolt_qt.m4: Minor fixes and clean-ups (fanquake)
...
- #11297 `16e4184` Make sure ~/.revolt doesn't exist before build (MeshCollider)
...
- #10871 `c9a4aa8` Handle getinfo in revolt-cli w/ -getinfo (revival of #8843) (achow101)
- #11419 `093074b` Utils: Fix launchctl not being able to stop revoltd (OmeGak)
...
- #10529 `331352f` Improve revoltd systemd service file (Flowdalic)
