...
Revolt Core version 23.0 is now available from:
...
using the Linux kernel, macOS 10.15+, and Windows 7 and newer.  Revolt
...
- Before this release, Revolt Core had a strong preference to try to connect only to peers that listen on port 8333. As a result of that, Revolt nodes listening on non-standard ports would likely not get any Revolt Core peers connecting to them. This preference has been removed. (#23542)
...
Revolt Core release binaries for Linux now include experimental tracepoints which
...
- The top-level fee fields `fee`, `modifiedfee`, `ancestorfees` and `descendantfees`
  ...
  fields in the `fees` object are denominated in RVT. (#22689)
...
* On startup, the list of banned hosts and networks (via `setban` RPC) in
  `banlist.dat` is ignored and only `banlist.json` is considered. Revolt Core
...
- CLI `-addrinfo` now returns a single field for the number of `onion` addresses
  ...
  for Tor V2 addresses was removed from Revolt Core in 22.0. (#22544)
