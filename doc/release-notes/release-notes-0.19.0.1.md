Revolt Core version 0.19.0.1 is now available from:
...
Revolt Core is supported and extensively tested on operating systems using
the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not recommended
to use Revolt Core on unsupported systems.
...
to open the Revolt Core .dmg. This is due to new signing requirements
imposed by Apple, which the Revolt Core project does not yet adhere too.
...
- [Reduce memory](https://github.com/bitcoin/bitcoin/blob/master/doc/reduce-memory.md)
  suggests configuration tweaks for running Revolt Core on systems with
...
- In 0.18.0, a `./configure` flag was introduced to allow disabling BIP70 support in the GUI (support was enabled by default). In 0.19.0, this flag is now __disabled__ by default. If you want to compile Revolt Core with BIP70 support in the GUI, you can pass `--enable-bip70` to `./configure`. (#15584)
...
- `generate` is now removed after being deprecated in Revolt Core 0.18.
...
- BIP 61 reject messages were deprecated in v0.18. They are now disabled
  ...
  future version of Revolt Core. (#14054)

- To eliminate well-known denial-of-service vectors in Revolt Core,
  ...
  This prevents Revolt Core from sending the BIP111 NODE_BLOOM service
  ...
  of Revolt Core and nodes that have manually enabled BIP37 support,
...
- By default, Revolt Core will now make two additional outbound connections that are exclusively used for block-relay.  No transactions or addr messages will be processed on these connections. These connections are designed to add little additional memory or bandwidth resource requirements but should make some partitioning attacks more difficult to carry out. (#15759)
...
- `getblockchaininfo` no longer returns a `bip9_softforks` object.
  ...
  an additional `type` field describes how Revolt Core determines
...
- When fetching a transaction announced by multiple peers, previous versions of
  Revolt Core would sequentially attempt to download the transaction from each
...
- If a Tor hidden service is being used, Revolt Core will be bound to
...
- When creating a transaction with a fee above `-maxtxfee` (default 0.1
  RVT), the RPC commands `walletcreatefundedpsbt` and
  `fundrawtransaction` will now fail instead of rounding down the fee.
  Be aware that the `feeRate` argument is specified in RVT per 1,000
...
- A new wallet flag `avoid_reuse` has been added (default off). When
  ...
  partial spends" (added in Revolt Core v0.17.0), this can eliminate a
...
- #16521 Use the default maxfeerate value as RVT/kB (Remagpie)
...
- #15896 feature_filelock, interface_revolt_cli: Use PACKAGE_NAME in messages rather than hardcoding Revolt Core (luke-jr)
...
- #16929 follow-up to rpc: default maxfeerate value as RVT/kB (jonatack)
