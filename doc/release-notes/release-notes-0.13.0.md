Revolt Core version 0.13.0 is now available from:
...
For example:
    ...
    $ src/revolt-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Revolt Core code base has
...
HD wallets are incompatible with older versions of Revolt Core.
...
proposed parameters, a future release of Revolt Core is expected that
...
The two phases are distinct in the Revolt-Qt GUI. During the first one,
...
- #7550 `8b958ab` Input-from-stdin mode for revolt-cli (laanwj)
...
- #8153 `75ec320` `fundrawtransaction` feeRate: Use RVT/kB (MarcoFalke)
...
- #7613 `3798e5d` Add autocomplete to revolt-qt's console window (GamerSg)
...
- #8207 `f7a403b` Add a link to the Revolt-Core repository and website to the About Dialog (MarcoFalke)
...
- #8047 `37f9a1f` Test_framework: Set wait-timeout for revoltd procs (MarcoFalke)
...
- #8164 `0f24eaf` [Revolt-Tx] fix missing test fixtures, fix 32bit atoi issue (jonasschnelli)
...
- #7744 `d8e862a` test_framework: detect failure of revoltd startup (laanwj)
...
- #7975 `fc23fee` Update revolt-core GitHub links (MarcoFalke)
