Revolt Core version 0.12.0 is now available from:
...
backwards-compatible with pre-0.10 versions of Revolt Core or other software:
...
ECDSA signatures inside Revolt Transactions now use validation using
...
Previous versions of Revolt Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Revolt Core was to raise the default minimum
relay fee.

Revolt Core 0.12 will have a strict maximum size on the mempool. The
...
Revolt Core 0.12 also introduces new default policy limits on the length and
...
Revolt Core 0.12 nodes. Revolt Core will only allow replacement of
...
Note that the wallet in Revolt Core 0.12 does not yet have support for
...
This allows running revoltd without having to do any manual configuration.
...
Revolt Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Revolt Core will relay transactions with
insufficient fees depending on the setting of `-limitfreerelay=<r>` (default:
`r=15` kB per minute) and `-blockprioritysize=<s>`.

In Revolt Core 0.12, when mempool limit has been reached a higher minimum
...
entirely in Revolt Core 0.13, and it is at this time undecided whether the
...
Revolt Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Revolt Core automatically creates a hidden service to listen on, without
manual configuration. Revolt Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Revolt Core is listening, and
...
Revoltd can now (optionally) asynchronously notify clients through a
...
(default: `0.0002` RVT/kB).

At all times, Revolt Core will cap fees at `-maxtxfee=<x>` (default:
0.10) RVT.
Furthermore, Revolt Core will never create transactions paying less than
...
Restarting revoltd will no longer clear out the list of banned peers; instead
...
- #6600 `ef4945f` Include revolt-tx binary on Debian/Ubuntu (Zak Wilcox)
...
- #6390 `6a73d66` tests: Fix revolt-tx signing test case (Wladimir J. van der Laan)
...
- #6730 `cb4d6d0` build: Remove dependency of revolt-cli on secp256k1 (Wladimir J. van der Laan)
...
- #6504 `b6fee6b` Rationalize currency unit to "RVT" (Ross Nicoll)
