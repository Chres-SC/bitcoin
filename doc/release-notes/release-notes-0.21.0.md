...
Revolt Core version 0.21.0 is now available from:
...
- The Tor onion service that is automatically created by setting the
  ...
  `-datadir`) and can be removed if not needed. Revolt Core will no longer
...
- A new `revolt-cli -netinfo` command provides a network peer connections
...
- A new `revolt-cli -generate` command, equivalent to RPC `generatenewaddress`
  followed by `generatetoaddress`, can generate blocks for command line testing
  purposes. This is a client-side version of the former `generate` RPC. See the
  help for details. (#19133)

- The `revolt-cli -getinfo` command now displays the wallet name and balance for
  each of the loaded wallets when more than one is loaded (e.g. in multiwallet
  mode) and a wallet is not specified with `-rpcwallet`. (#18594)

- The `connections` field of `revolt-cli -getinfo` is now expanded to return a JSON
...
- The `startupnotify` option is used to specify a command to
  execute when Revolt Core has finished with its startup
  sequence. (#15367)
...
- The `-salvagewallet` startup option has been removed. A new `salvage` command
  has been added to the `revolt-wallet` tool which performs the salvage
...
wallet option), through the `revolt-cli createwallet` or `revolt-wallet
...
Users who already have a Revolt Core wallet can continue to use it as they did before without
any change in behavior. Newly created Legacy Wallets (which remains the default type of wallet) will
behave as they did in previous versions of Revolt Core.
...
The change to using descriptors changes the default derivation paths used by Revolt Core
...
- A new `fee_rate` parameter/option denominated in satoshis per vbyte (sat/vB)
  ...
  `walletcreatefundedpsbt` still exists for setting a fee rate in RVT per 1,000
  vbytes (RVT/kvB), but it is expected to be deprecated soon to avoid
  confusion. For these RPCs, the fee rate error message is updated from RVT/kB
  to sat/vB and the help documentation in RVT/kB is updated to RVT/kvB. The
  `send` and `sendtoaddress` RPC examples are updated to aid users in creating
  transactions with explicit fee rates. (#20305, #11413)

- The `bumpfee` RPC `fee_rate` option is changed from RVT/kvB to sat/vB and the
  ...
  difference between RVT/kvB and sat/vB units means that a transaction with a
  fee rate mistakenly calculated in RVT/kvB rather than sat/vB should raise an
...
- #20198 Show name, format and if uses descriptors in revolt-wallet tool (jonasschnelli)
...
- #19133 rpc, cli, test: add revolt-cli -generate command (jonatack)
...
- #20055 Set HTTP Content-Type in revolt-cli (laanwj)
...
### Build system
- #18504 Drop revolt-tx and revolt-wallet dependencies on libevent (ryanofsky)
...
- #18653 add coverage for revolt-cli -rpcwait (jonatack)
...
- #18739 Document how to fuzz Revolt Core using Honggfuzz (practicalswift)
...
- #18817 Document differences in revoltd and revolt-qt locale handling (practicalswift)
