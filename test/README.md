This directory contains integration tests that test revoltd and its
...
revoltd and revolt-qt by interacting with them through the RPC and P2P
interfaces.
- [util](/test/util) which tests the utilities (revolt-util, revolt-tx, ...).
...
Before tests can be run locally, Revolt Core must be built.  See the [building instructions](/doc#building) for help.
...
The P2P and RPC ports used by the revoltd nodes-under-test are chosen to make
conflicts with other processes unlikely. However, if there is another revoltd
process running on the system (perhaps from a previous test which hasn't successfully
killed all its revoltd nodes), then there may be a port conflict which will
cause the test to fail. It is recommended that you run the tests on a system
where no other revoltd processes are running.

On linux, the test framework will warn if there is another
revoltd process running when the tests are started.

If there are zombie revoltd processes after test failure, you can kill them
by running the following commands. **Note that these commands will kill all
revoltd processes running on the system, so should not be used if any non-test
revoltd processes are being run.**
...
sure revoltd processes are stopped as above):
...
- when run by [our CI (Continuous Integration)](/ci/README.md), no logs are output to the console. However, if a test
  fails, the `test_framework.log` and revoltd `debug.log`s will all be dumped
...
`test_framework.log` and revoltd `debug.log`s can be combined into a single
...
call methods that interact with the revoltd nodes-under-test.

If further introspection of the revoltd instances themselves becomes
...
```bash
cat /tmp/user/1000/testo9vsdjo3/node1/regtest/revoltd.pid
```

Then you can use the pid to start `gdb`:

```bash
gdb /home/example/revoltd <pid>
```
